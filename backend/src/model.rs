...
impl ModelController {
    ...
    pub async fn get(&self, id: u32) -> Result<Option<Ticket>> {
        ...
        if ticket.is_some() {
            Ok(ticket.unwrap().clone())
        } else {
            Err(Error::TicketGetFailIDNotFound { id })
        }
    }
    ...
    pub async fn update(
        ...
        let ticket = store
            .get_mut(id as usize)
            .and_then(|ticket| ticket.replace(new_ticket));
    ...
    pub async fn delete(&self, id: u32) -> Result<Option<Ticket>> {
        let mut store: std::sync::MutexGuard<Vec<Option<Ticket>>> =
            self.tickets_store.lock().unwrap();

        let ticket = store
            .get_mut(id as usize)
            .and_then(|ticket: &mut Option<Ticket>| ticket.take());
